...
func NewDisplay(data, clock, latch rpio.Pin, dir int) *RealDisplay {
	data.Output()
	clock.Output()
	latch.Output()

	d := &RealDisplay{data, clock, latch, dir}
...
func (d *RealDisplay) ShowText(text string) error {
...
func (d *RealDisplay) displayData(data [8]string, timeout time.Duration) {
	// turn on
...
func (d *RealDisplay) displayMatrix(matrix [8]string) {
	for i := 0; i < d.dir; i++ {
...
func (d *RealDisplay) sendDataInt(address, value int) {
...
func (d *RealDisplay) processData(value string) {
	fmt.Println(value)
